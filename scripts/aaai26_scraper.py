#!/usr/bin/env python3
"""
AAAI-26 COMPLETE paper scraper (official source: ojs.aaai.org, Proceedings Vol. 40)
-----------------------------------------------------------------------------------
Output:
...
Usage:
  ...
  python aaai26_scraper.py              # fast: title-based classification
  python aaai26_scraper.py --abstract   # thorough: title + abstract (4000+ extra requests, slow)
"""
...
def find_vol40_issues(first_id=683, scan_to=800):
    """
    ...
    for iid in range(first_id, scan_to + 1):
        ...
        if re.search(r"Vol\.?\s*40\b", title):
            issues[str(iid)] = title
            misses_after_last = 0
            print(f"  + {iid}: {title}")
        elif issues:
            misses_after_last += 1
            if misses_after_last >= 15:   # 15 non-Vol.40 IDs in a row -> finished
                break
        time.sleep(DELAY)
    return issues
...
def main():
    ...
    print("Finding AAAI-26 (Vol. 40) issues ...")
    issues = find_vol40_issues()
    if not issues:
        sys.exit("No Vol. 40 issues found — check internet / site layout.")
    print(f"  {len(issues)} issues found (official note says 48)."
          + ("" if len(issues) == 48 else "  !! NOT 48 — check before trusting the list!"))
    ...
    ds.to_csv("AAAI26_dataset_papers.csv", index_label="No")
    other.to_csv("AAAI26_other_papers.csv", index_label="No")
    ...
    summary = pd.DataFrame({
        ...
        "Value": [len(issues), ", ".join(failed) or "None", len(df), len(ds), len(other),
                  ...
                  "https://ojs.aaai.org/index.php/AAAI/issue/archive (Vol. 40)"],
    })
    with pd.ExcelWriter("AAAI26_papers.xlsx", engine="openpyxl") as xw:
