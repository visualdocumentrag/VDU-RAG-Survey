#!/usr/bin/env python3
"""
CVPR 2026 COMPLETE paper scraper (official source: openaccess.thecvf.com)
-------------------------------------------------------------------------
...
Usage (Colab):
  ...
  !python cvpr26_scraper.py              # fast: title only (~2 min)
  !python cvpr26_scraper.py --abstract   # thorough: title + abstract (parallel, ~20-40 min)
"""
...
TRACKS = {"Main": "CVPR2026", "Findings": "CVPR2026_findings"}
...
def main():
    ...
    ds.to_csv("CVPR26_dataset_papers.csv", index_label="No")
    other.to_csv("CVPR26_other_papers.csv", index_label="No")
    ...
    summary = pd.DataFrame({
        ...
        "Value": [len(df), int((df.Track == "Main").sum()), int((df.Track == "Findings").sum()),
                  ...
                  "https://openaccess.thecvf.com/CVPR2026 (+ _findings)"]})
    with pd.ExcelWriter("CVPR26_papers.xlsx", engine="openpyxl") as xw:
