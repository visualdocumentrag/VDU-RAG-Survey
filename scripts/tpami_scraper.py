#!/usr/bin/env python3
"""
IEEE TPAMI COMPLETE paper scraper (any year: --year 2025, 2026, ...)
--------------------------------------------------------------------
...
Usage (Colab):
  ...
  !python tpami_scraper.py --year 2025   # any year; ~2-4 minutes
"""
...
SKIP = re.compile(r"^(?:Front Cover|Back Cover|Table of Contents|IEEE Transactions on Pattern|"
                  r"Information for Authors|Editorial|Corrections? to|Erratum|Retraction(?:\s|:)|"
...
def fetch_crossref(year):
    rows, seen = [], set()
    for issn in ISSNS:
        cursor, n0 = "*", len(rows)
        while True:
            j = get(CROSSREF.format(issn), {"filter": f"from-pub-date:{year}-01-01,until-pub-date:{year}-12-31",
...
def main():
    ...
    print(f"Step 1: TPAMI {YEAR} articles from Crossref ...")
    ...
    summary = pd.DataFrame({
        "Item": [f"Total articles ({YEAR})", f"  in an issue (Vol. {YEAR - 1978})", "  Early Access", "Removed non-paper items",
        ...
        "Value": [len(df), int((df.Status == "Issue").sum()), int((df.Status == "Early Access").sum()),
                  ...
                  f"Crossref (ISSN 0162-8828 / 1939-3539, pub-date {YEAR}) + OpenAlex"]})
    with pd.ExcelWriter(f"TPAMI{YEAR % 100}_papers.xlsx", engine="openpyxl") as xw:
