#!/usr/bin/env python3
"""
ECCV 2026 COMPLETE accepted-paper scraper (official source: eccv.ecva.net virtual site)
----------------------------------------------------------------------------------------
ECCV 2026, Malmo, Sept 8-13, 2026. Springer proceedings / OpenReview are not script-friendly,
so the official ECVA virtual-site data file is used (same system that worked for ICLR/ICML).

Steps: 1) download the official data file (no login)
       2) remove duplicate Oral/Spotlight event rows (same paper listed twice)
       3) if the file has no abstracts, read each paper's eccv.ecva.net page (parallel, ~15-25 min)
Output: ECCV26_papers.xlsx -> Dataset_Papers | Other_Papers | VDU_RAG_Candidates | Summary
Usage (Colab):
  !pip install requests beautifulsoup4 pandas openpyxl
  !python eccv_scraper.py --year 2024              # full (with abstracts)
  !python eccv_scraper.py --year 2024 --no-abstract   # 1 minute, titles only
"""
...
DATA = SITE + "/static/virtual/data/eccv-{}-orals-posters.json"   # year filled in main()
OFFICIAL_TOTAL = "not published"
...
def load_rows():
    ...
    raw = r.json()
    json.dump(raw, open("eccv_raw.json", "w"))
    ...
    for it in items:
        ...
        new = {"Title": title, "Authors": au, "Decision": dec,
               ...
               "ECCV_Page": vs, "Abstract": " ".join(BeautifulSoup(abstract, "html.parser")
                                                      .get_text(" ").split())}
        old = papers.get(key)
        if old is None:
            papers[key] = new
            ...
            if not old["ECCV_Page"]:
                old["ECCV_Page"] = new["ECCV_Page"]
...
def main():
    ...
    print(f"Downloading ECCV {Y} data file: {DATA}")
    ...
    if missing.any() and not args.no_abstract:
        todo = [u for u in df.loc[missing, "ECCV_Page"] if u]
        ...
        for rnd in range(2):
            ...
            time.sleep(30)
        failed = len(todo)
        df.loc[missing, "Abstract"] = df.loc[missing, "ECCV_Page"].map(got).fillna("")
    ...
    with pd.ExcelWriter(f"ECCV{Y % 100}_papers.xlsx", engine="openpyxl") as xw:
