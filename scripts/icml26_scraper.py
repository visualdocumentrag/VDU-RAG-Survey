#!/usr/bin/env python3
"""
ICML 2026 COMPLETE accepted-paper scraper (official source: icml.cc virtual site)
--------------------------------------------------------------------------------
Official numbers (ICML 2026): 6,352 accepted papers (536 spotlight, 168 oral).
PMLR proceedings for ICML 2026 are not published yet, OpenReview blocks Colab -> virtual site.

Steps: 1) download the official data file (no login)
       2) remove duplicate Oral/Spotlight event rows (same paper listed twice)
       3) if the file has no abstracts, read each paper's icml.cc page (parallel, ~15-25 min)
Output: ICML26_papers.xlsx -> Dataset_Papers | Other_Papers | VDU_RAG_Candidates | Summary
Usage (Colab):
  !pip install requests beautifulsoup4 pandas openpyxl
  !python icml26_scraper.py              # full (with abstracts)
  !python icml26_scraper.py --no-abstract   # 1 minute, titles only
"""
...
SITE = "https://icml.cc"
DATA = SITE + "/static/virtual/data/icml-2026-orals-posters.json"
OFFICIAL_TOTAL = 6352
...
def load_rows():
    ...
    raw = r.json()
    json.dump(raw, open("icml26_raw.json", "w"))
    ...
    for it in items:
        ...
        new = {"Title": title, "Authors": au, "Decision": dec,
               ...
               "ICML_Page": vs, "Abstract": " ".join(BeautifulSoup(abstract, "html.parser")
                                                      .get_text(" ").split())}
        old = papers.get(key)
        if old is None:
            papers[key] = new
            ...
            if not old["ICML_Page"]:
                old["ICML_Page"] = new["ICML_Page"]
...
def main():
    ...
    print("Downloading ICML 2026 data file ...")
    ...
    if missing.any() and not args.no_abstract:
        todo = [u for u in df.loc[missing, "ICML_Page"] if u]
        ...
        for rnd in range(2):
            ...
            time.sleep(30)
        failed = len(todo)
        df.loc[missing, "Abstract"] = df.loc[missing, "ICML_Page"].map(got).fillna("")
    ...
    with pd.ExcelWriter("ICML26_papers.xlsx", engine="openpyxl") as xw:
