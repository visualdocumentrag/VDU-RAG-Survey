#!/usr/bin/env python3
"""
ICLR COMPLETE accepted-paper scraper (official source: iclr.cc virtual site), any year
-------------------------------------------------------------------------------------
Same system that worked for ICML 2026 / NeurIPS 2025. Removes duplicate Oral rows (fake ids),
and reads each paper page for the abstract when the data file has none (ICLR 2026 file had none).

Steps: 1) download the official data file (no login)
       2) remove duplicate Oral/Spotlight event rows (same paper listed twice)
       3) if the file has no abstracts, read each paper's iclr.cc page (parallel, ~15-25 min)
Output: ICLR25_papers.xlsx -> Dataset_Papers | Other_Papers | VDU_RAG_Candidates | Summary
Usage (Colab):
  !pip install requests beautifulsoup4 pandas openpyxl
  !python iclr_scraper.py --year 2025        # full (with abstracts)
  !python iclr_scraper.py --year 2025 --no-abstract
"""
...
SITE = "https://iclr.cc"
DATA = SITE + "/static/virtual/data/iclr-{}-orals-posters.json"   # year filled in main()
...
def load_rows():
    ...
    raw = r.json()
    json.dump(raw, open("iclr_raw.json", "w"))
    ...
    for it in items:
        ...
        new = {"Title": title, "Authors": au, "Decision": dec,
               ...
               "ICLR_Page": vs, "Abstract": " ".join(BeautifulSoup(abstract, "html.parser")
                                                      .get_text(" ").split())}
        old = papers.get(key)
        if old is None:
            papers[key] = new
            ...
            if not old["ICLR_Page"]:
                old["ICLR_Page"] = new["ICLR_Page"]
...
def main():
    ...
    print(f"Downloading ICLR {Y} data file: {DATA}")
    ...
    if missing.any() and not args.no_abstract:
        todo = [u for u in df.loc[missing, "ICLR_Page"] if u]
        ...
        for rnd in range(2):
            ...
            time.sleep(30)
        failed = len(todo)
        df.loc[missing, "Abstract"] = df.loc[missing, "ICLR_Page"].map(got).fillna("")
    ...
    with pd.ExcelWriter(f"ICLR{Y % 100}_papers.xlsx", engine="openpyxl") as xw:
