#!/usr/bin/env python3
"""
KDD 2026 COMPLETE paper scraper
-------------------------------
KDD 2026 has TWO proceedings volumes in the ACM Digital Library (cycle 1 = Aug 2025 deadline,
cycle 2 = Feb 2026 deadline). ACM DL blocks scripts, so:
  1) find both KDD '26 proceedings volumes + their ISBNs  <- Crossref (ACM's own DOI registry)
  2) every paper in those volumes, with abstracts           <- Crossref (filter by ISBN)
  3) abstracts still missing                                <- OpenAlex, looked up by DOI
Output: KDD26_papers.xlsx -> Dataset_Papers | Other_Papers | VDU_RAG_Candidates | Summary
Usage (Colab):
  !pip install requests pandas openpyxl
  !python kdd26_scraper.py          # ~2-4 minutes
"""
import re, sys, time
import requests
import pandas as pd
...
KNOWN_VOLUMES = ["10.1145/3770854"]          # KDD '26 Vol. 1 (verified on ACM DL)


def crossref_volumes():
    """Return {proceedings DOI: (title, [ISBNs])} for every KDD '26 volume."""
    vols = {}
    j = get(CROSSREF, {"filter": "type:proceedings,prefix:10.1145,from-pub-date:2025-06-01",
                       "query.bibliographic": "SIGKDD Conference on Knowledge Discovery and Data Mining",
                       "rows": 50})
    for w in (j or {}).get("message", {}).get("items", []):
        ev = w.get("event", {}) or {}
        if "KDD '26" in (ev.get("acronym") or "") or "KDD '26" in (ev.get("name") or ""):
...
def crossref_papers(vol_doi, isbns, vol_title):
    ...
    strategies.append(("container search", {"query.container-title": vol_title,
                                            "filter": "prefix:10.1145,from-pub-date:2025-06-01"}))
    rows, seen = [], set()
...
def main():
    print("Step 1: KDD '26 proceedings volumes (Crossref) ...")
    vols = crossref_volumes()
    for d, (t, i) in vols.items():
        print(f"  {d}  ISBN {i}  {t[:90]}")
    if not vols:
        sys.exit("No KDD '26 volume found on Crossref -> send a screenshot.")
    ...
    with pd.ExcelWriter("KDD26_papers.xlsx", engine="openpyxl") as xw:
